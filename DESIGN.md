...
### Navigation
- *Sections.* Every destination belongs to one of four sections, in this order: Föreningen (Adressbok, Nyheter, Evenemang, Chatt, Dokument, Motioner), Huset (Ärenden, Bokningar, Nycklar, Lägenhetspärm, Andrahand), Styrelsen (Styrelsens post, Skriv nyheter, Webbplatsen, Stämmor, Avgifter, Debiteringar, Dataskydd) and Inställningar (Inställningar, Tillägg, Anslutna appar). A section the account has nothing in is not shown; a section with one destination is that destination's own sign.
- *Band (from 1024px).* Section signs in the dark band - 13px/600 uppercase, `--tavla-ink-2` at rest, full band height, a small chevron on a sign that opens; Inställningar at the band's right end. A sign opens its destinations under it on the board surface: 44px rows, names 15/500 `--tavla-ink`, 1px `--tavla-line` rails, one shadow, 8px lower corners. The sign whose section holds the current page gets `--tavla-ink` + 3px `--massing-tavla` underline; the current row a 3px `--massing-tavla` leading edge. Counters as small brass plates; a section sign carries the sum of its destinations'.
- *Mobile (below 1024px).* A 4-slot bottom bar on `--tavla`: three destinations for the account (the board: Adressbok, Ärenden, Chatt; residents: Nyheter, Ärenden, Bokningar) and Meny in the fourth, lettered as sign chips. Active in `--massing-tavla` with a 3px top edge; Meny carries it when the page is not one of the three. Meny opens a sheet rising from the bar that reads like the name board, floor by floor: each section a floor row on `--tavla-2`, its destinations as rows beneath. The room behind is inert while it is open.
- *Focus* on the band and the bar is `--massing-tavla` (the On-Board Variant Rule).
