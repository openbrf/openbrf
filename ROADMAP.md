...
- [ ] Event calendar with sign-ups
  - Landed: the event series the board arranges, with its category, its
    ...
    sign-up per occurrence with the places counted per date and a claim the
    database and a lock decide, withdrawal as a dated close that frees the
    place again, the roll-call behind the managing capability with a
    protected person counted and never named, the attending capability, the
    data subject access report section and the sign-up purge with its
    legal-hold check
