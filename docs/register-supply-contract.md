...
**Pledges are in this file, although they are not in the obligation ledger.** The
...
their own; a pledge belongs to an apartment and not to a holder; the
...
times, silently dropping the third co-holder, or leave the pledges out.
...
| `PLEDGE`      | one per pledge still noted     | Pantsättningar (2 kap. 6 §)           |

`HOLDER` and `PLEDGE` rows point at their `APARTMENT` row through
`apartmentKey`.

Rows come out in a stable order: the association first, then each apartment
followed by its holders and its pledges, apartments in the order the address book
...
| -------------------------------- | ------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `recordType`                     | every row                       | -                        | Open BRF's own. `ASSOCIATION`, `APARTMENT`, `HOLDER` or `PLEDGE`.                                                                                                                                                                                                                                                                                                                                                                             |
| `apartmentKey`                   | `APARTMENT`, `HOLDER`, `PLEDGE` | -                        | Open BRF's own. The apartment as street, number and apartment number, which is how the apartment register designates it and is unique within an association. It exists so a holder or a pledge can name the apartment it belongs to; it is **not** the beteckning of 2 kap. 3 § första stycket 1.                                                                                                                                             |
...
| `pledgeCreditor`                 | `PLEDGE`                        | 2 kap. 6 § 1 1           | Panthavarens namn, as the association noted it.                                                                                                                                                                                                                                                                                                                                                                                               |
| `pledgeNotedOn`                  | `PLEDGE`                        | -                        | Open BRF's own. The anteckningsdag in the association's own lägenhetsförteckning, which is not itself one of the register's fields but is what shows the pledge existed before the act came into force (Lag (2026:485) 11 §).                                                                                                                                                                                                                 |
...
name and their personal identity number, which are what identifies a holder in a
register keyed on exactly those, and leaves the three postal columns empty with
`holderProtectedPersonalData` set to `yes` beside them.
...
- **A released pledge.** A pantsättning that no longer applies is deregistered by
