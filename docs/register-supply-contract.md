...
| `associationPropertyDesignation`     | `ASSOCIATION`                 | 2 kap. 4 § andra stycket | Fastighetsbeteckning, from the association's own authoritative record of it and never from the prose published on the broker page. Andra stycket makes this field conditional: it is reported in place of the lagfarts- och tomträttsinnehav of första stycket 4 where the association's buildings stand on land it neither owns nor holds with tomträtt. Which of those cases the association is in is `associationLandTenure` below.                                                                    |
| `associationLandTenure`              | `ASSOCIATION`                 | -                        | Open BRF's own. `OWNERSHIP`, `SITE_LEASEHOLD`, `OTHER`, or empty where the board has not recorded it. Not the innehav of första stycket 4, which is not recorded - it is which of that section's two cases applies, and it is here because the two columns after it are empty in every case but `OTHER`. An empty conditional column with nothing saying why cannot be told from one the register lost, which is the reasoning `holderProtectedPersonalData` carries about the address columns beside it. |
| `associationTaxAssessmentUnitNumber` | `ASSOCIATION`                 | 2 kap. 4 § andra stycket | Taxeringsenhetsnummer, reported with the fastighetsbeteckning and the fastighetstyp in place of the lagfarts- och tomträttsinnehav. Empty unless `associationLandTenure` is `OTHER`, which the database enforces with a CHECK rather than leaving to this file. Free text: the förordning names the field and does not define its form, and 2 kap. 2 § and 5 kap. 1-2 §§ leave the föreskrifter to Lantmäteriet.                                                                                          |
| `associationPropertyType`            | `ASSOCIATION`                 | 2 kap. 4 § andra stycket | Fastighetstyp, on the same condition and the same reading. Free text for a stronger reason: the nearest statutory classification, the type of taxeringsenhet under fastighetstaxeringslagen (1979:1152) 4 kap. 5 §, classifies the taxeringsenhet rather than the property, so an enum here would be Open BRF deciding what the field means. The board records what its own property record states.                                                                                                       |
...
| 2 kap. 4 § 4, lagfarts- och tomträttsinnehav                                                                       | Not recorded. Which of the section's two cases the association is in **is** recorded, on the association singleton and reported as `associationLandTenure`; the innehav itself is not. It is not read off the association facts page, whose site-leasehold boolean reads false as "owns the land" and therefore has no value at all for the case andra stycket turns on. |
