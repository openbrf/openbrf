...
**It is not an accounting file.** There are no account numbers, no vouchers
(verifikationer), no debit and credit and no balances. Open BRF holds no ledger and no
...
**It is not an invoice.** An annual fee is exempt from value added tax:
mervärdesskattelagen (2023:200) 10 kap. 35 § exempts "överlåtelse och upplåtelse
av arrenden, hyresrätter, bostadsrätter, tomträtter, servitutsrätter och andra
rättigheter till fastigheter", which is what the fee is paid for. A line that
10 kap. 36 § takes back out of that exemption - a parking space (point 5), a
storage box (point 6), advertising space (point 7) - needs a real invoice, and
so do commercial premises taxed under the voluntary taxation that same section's
closing sentence points to in ML 12 kap. Open BRF produces none of them.
`docs/fee-notice-contract.md` states that in full.
...
| `#VER` series and number                                    | may be empty | A feeder system writing 4I may leave both to the receiving program, so Open BRF would not have to invent voucher series (verifikationsserier) either.                       |
...
apostrophe** - one beginning `=`, `+`, `-`, `@`, a tab or a carriage return -
so that a `reason` a board typed cannot run as a formula in the books of
whoever opens the file. **A number is left exactly as it was**, sign included,
because an amount is what this file is read for and a figure turned into text
would no longer add up. Dates need no exemption: they are written `YYYY-MM-DD`,
which begins with a digit, as does every payment reference.
...
**The fee half of this file names nobody at all.** The apartment is the party
the fee is fixed on: the annual fee is owed for the tenant-ownership (BRL 7 kap.
14 §) and how it falls between the apartments is a matter for the bylaws (BRL 9
kap. 5 § första stycket 5), while fixing the amounts is the board's own task
(BRL 9 kap. 13 §). Who holds the apartment is on the fee notice document rather
than here.
...
of that period says - so a file is produced again rather than edited. The screen
states the day each file was produced, on the association's own calendar, and
the audit log records when each one was taken.
