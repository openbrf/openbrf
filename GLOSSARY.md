...
| sida | **page** | One page on the association's own website, at its own address directly under the domain root. Written by the board, published or held as a draft, and either public or readable by anyone signed in - a page nobody may read is answered exactly as one that does not exist. Its body is a list of blocks and never markup, so the renderer decides what a block becomes and no stored page can carry a script or a third-party address. Service tier: the cooperative's own writing about itself, not register content. |
