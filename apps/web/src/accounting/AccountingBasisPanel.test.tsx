 ...
 *
 * Mocked rather than served, because what the panel does with the answer is
 * the thing under test: the endpoint requires both capabilities, and a board
 * member holding one of them must not be offered a control the server refuses.
 */
...
const BOTH = ["fees:manage", "memberCharges:manage"];
...
beforeEach(() => {
  vi.resetAllMocks();
  capabilities.mockReturnValue(BOTH);
...
describe("AccountingBasisPanel", () => {
  ...
  it("offers the file under the period's own name once it is produced", async () => {
    ...
    // The day it was produced, stated beside the file the way every other
    // document in this product carries its stamp.
    expect(
      screen.getByText(
        "Bokföringsunderlag - 2026-01-01 till 2026-12-31 - framställt 2026-09-18",
      ),
    ).toBeTruthy();
  ...
  it.each([["fees:manage"], ["memberCharges:manage"], []])(
    ...
    (...held) => {
      /*
       * The export needs both halves of the money, so a seat holding one of them
       * is offered no control at all rather than one the server then refuses.
       * The empty case is the viewer endpoint not having answered yet, and it
       * takes the same path: the panel appears when the answer does.
       */
      capabilities.mockReturnValue(held.flat());

      const { container } = render(
        <AccountingBasisPanel onRefused={vi.fn()} />,
      );
