            ...
            .filter(
              (comment) =>
                ...
                (requestedIds === undefined
                  ...
                  : requestedIds.includes(comment.authorPersonId)),
            )
...
describe("choosing who a run erases for", () => {
  ...
  it("reaches a person a granted request names from behind a run's worth of expiries", async () => {
     ...
     * comments sit past the date the board granted while the request says it
    ...
    const { service } = build({
      comments: [
        ...expiredPersonIds.map(expiredCommentFor),
        // Written an hour ago, so nothing but the request selects them, and
